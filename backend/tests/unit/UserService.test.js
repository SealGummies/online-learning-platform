...
describe("UserService Unit Tests", () => {
  ...
  test("should get student stats", async () => {
    const userData = {
      firstName: "Test",
      lastName: "Student",
      email: `student-stats-${Date.now()}@example.com`,
      password: "testpassword",
      role: "student",
    };
    const user = await UserService.createUser(userData);
    testUser = user;
    const stats = await UserService.getStudentStats(user._id);
    expect(stats.totalEnrollments).toBeDefined();
    expect(stats.inProgressCourses).toBeDefined();
    expect(stats.completedCourses).toBeDefined();
    expect(stats.averageCompletion).toBeDefined();
  });

  test("should get instructor stats", async () => {
    const userData = {
      firstName: "Test",
      lastName: "Instructor",
      email: `instructor-stats-${Date.now()}@example.com`,
      password: "testpassword",
      role: "instructor",
    };
    const user = await UserService.createUser(userData);
    testUser = user;
    const stats = await UserService.getInstructorStats(user._id);
    expect(stats.totalCourses).toBeDefined();
    expect(stats.activeCourses).toBeDefined();
    expect(stats.totalStudents).toBeDefined();
  });

  test("should throw error when getting student stats for non-student", async () => {
    const userData = {
      firstName: "Test",
      lastName: "Instructor",
      email: `instructor-not-student-${Date.now()}@example.com`,
      password: "testpassword",
      role: "instructor",
    };
    const user = await UserService.createUser(userData);
    testUser = user;
    await expect(UserService.getStudentStats(user._id)).rejects.toThrow("User is not a student");
  });

  test("should throw error when getting instructor stats for non-instructor", async () => {
    const userData = {
      firstName: "Test",
      lastName: "Student",
      email: `student-not-instructor-${Date.now()}@example.com`,
      password: "testpassword",
      role: "student",
    };
    const user = await UserService.createUser(userData);
    testUser = user;
    await expect(UserService.getInstructorStats(user._id)).rejects.toThrow("User is not an instructor");
  });
});
